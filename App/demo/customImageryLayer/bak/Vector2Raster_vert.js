...
bool coorEquals(vec2 p1,vec2 p2){
    if(p1.x==p2.x&&p1.y==p2.y)return true;
    return false;
}
bool coorEquals(vec3 p1,vec3 p2){
...
vec2 find_corner_dir(vec2 pt,vec2 prevPt,vec2 nextPt,float lineW){
    ...
    vec2 dir=crossPt-pt;
    if(length(dir)>lineW*lineScale*4.){
        vec2 dir=rotate90(prevPt,pt);
        return dir*lineW;
    }
    return dir;
...
vec2 lineBufferDir(vec2 pt,vec2 prevPt,vec2 nextPt,float lineW){
    ...
    if (coorEquals(position,nextPosition)) {
            centerDir = rotate90(prevPt, pt)*lineW;
            return centerDir;
    } else  if (coorEquals(position,prevPosition)) {
...
void main(){
    ...
    // vec2 posWC=lineBufferDir(pt,prevPt,nextPt,0.5*lineWidth*lineScale*sideMask)+pt;
    
    vec2 posWC=pt+bufferDir*lineWidth;
    
    vec2 posNor=windowCoordinates_normalize(posWC);
    textureCoords=posNor;
    gl_Position=vec4((textureCoords-.5)*2.,0.99,1.);
